...
function DiscountProductPage() {
  ...
  const discountedProducts = allProducts.filter((product) => {
    // Handle null or undefined discount
    if (!product.discount) return false;

    // Remove '%' and parse the string to a number
    const discountValue = parseFloat(product.discount.replace("%", ""));

    // Check if discountValue is a valid number and not zero
    return !isNaN(discountValue) && discountValue < 0; // Negative discounts indicate a price reduction
  });
