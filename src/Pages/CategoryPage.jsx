...
import products from "../card.json";
...
function CategoryPage() {
  ...
  const allProducts = [
    ...products.products_group_1,
    ...products.products_group_2,
    ...products.products_group_3,
  ];
  const filteredProducts = allProducts.filter(
    (product) => product.search_by_category === categoryName
  );
  ...
  return filteredProducts.length > 0 ? (
    <>
      ...
      >
        <div className="grid grid-cols-1 min-[650px]:grid-cols-2 justify-items-center gap-6 min-[1120px]:grid-cols-4 min-[1120px]:gap-16 px-6">
          {filteredProducts &&
      ...
      <div className="flex items-center justify-center mt-5 mb-10">
        <p className="text-2xl font-semibold text-center">
          {t("category.more_products")}
        </p>
      </div>
    </>
  ) : (
    <div className="flex flex-col items-center justify-center gap-4 text-center mt-16 mb-5">
      <p className="text-3xl font-bold mb-5">{t("category.no_products")}</p>
      <Link to="/">
        <div className="mb-5">
          <Button
            value={t("category.return_to_home")}
            variant="primary"
          ></Button>
        </div>
      </Link>
